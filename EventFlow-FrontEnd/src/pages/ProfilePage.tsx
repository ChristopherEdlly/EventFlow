...
export default function ProfilePage({ onBack }: ProfilePageProps) {
  ...
  const handleChangeEmail = async () => {
    if (!newEmail.trim()) {
      showNotification('Digite o novo email', 'error');
      return;
    }
    ...
    if (newEmail === profile?.email) {
      showNotification('O novo email é igual ao atual', 'error');
      return;
    }

    // Para contas locais, exigir senha
    if (profile?.authProvider === 'LOCAL' && !emailChangePassword) {
    ...
    try {
      ...
      // NOVA API: Requisitar mudança de email (não muda imediatamente)
      await api.requestEmailChange(newEmail, emailChangePassword || undefined);
      ...
      showNotification('Código enviado para o novo email! Verifique para completar a mudança.', 'success');

      // Recarregar perfil para mostrar pendingEmail
      await loadProfile();

      // Abrir dialog de verificação automaticamente
      setTimeout(() => {
      ...
      const apiError = err as ApiError;
      showNotification(apiError.message || 'Erro ao solicitar mudança de email', 'error');
    } finally {
  ...
  const handleCancelEmailChange = async () => {
    try {
      ...
      showNotification('Mudança de email cancelada', 'success');
      await loadProfile();
      setShowVerificationDialog(false);
    } catch (err) {
      const apiError = err as ApiError;
      showNotification(apiError.message || 'Erro ao cancelar mudança de email', 'error');
    }
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex items-start">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-medium text-amber-800 mb-1">
                  {profile.pendingEmail ? 'Mudança de email pendente' : 'Email não verificado'}
                </h3>
                <p className="text-sm text-amber-700 mb-3">
                  {profile.pendingEmail ? (
                    <>
                      Você solicitou mudar seu email para <strong>{profile.pendingEmail}</strong>.
                      Verifique o código enviado para completar a mudança.
                    </>
                  ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {profile.emailVerified ? (
                      ...
                      <p className="text-xs text-amber-600 mt-1">Email não verificado</p>
                    )}
                  </div>
                </div>
              ) : (
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3, delay: 0.1 }}
              className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 lg:sticky lg:top-6"
            >
              <h2 className="text-lg font-semibold text-neutral-900 mb-4">Visão Geral</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-neutral-500 mb-2">Tipo de Autenticação</label>
                  {getAuthProviderBadge(profile.authProvider)}
                </div>
                <div>
                  <label className="block text-xs font-medium text-neutral-500 mb-2">Cargo/Função</label>
                  {getRoleBadge(profile.role)}
                </div>
                <div>
                  <label className="block text-xs font-medium text-neutral-500 mb-2">Membro desde</label>
                  <p className="text-sm font-medium text-neutral-900">{formatDate(profile.createdAt)}</p>
                </div>
              </div>
            </motion.div>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-neutral-600 text-sm">
                  Você precisará verificar o novo email antes da mudança ser efetivada.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  value={profile.email}
                  disabled
                  className="w-full px-4 py-3 bg-gray-100 text-neutral-600 border border-neutral-300 rounded-lg cursor-not-allowed"
                />
              </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-6">
              <p className="text-sm text-blue-800">
                <strong>Importante:</strong> Após confirmar, enviaremos um código de verificação para o novo email. Você precisará verificá-lo para completar a mudança.
              </p>
            </div>
      ...
      {showVerificationDialog && profile && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full">
            <div className="flex items-start gap-4 mb-6">
              <div className="flex-shrink-0 flex items-center justify-center w-12 h-12 rounded-full bg-primary-100">
                ...
                <p className="text-neutral-600 text-sm">
                  {profile.pendingEmail ? (
                    <>
                      ...
                      <strong className="text-primary-600">{profile.pendingEmail}</strong>
                      <br />
                      <span className="text-xs text-neutral-500 mt-1 block">
                        Seu email atual ({profile.email}) será alterado após a verificação
                      </span>
                    </>
                  ) : (
                    <>
              ...
              <button
                onClick={() => {
                  if (profile.pendingEmail) {
                    // Se há email pendente, perguntar se quer cancelar a mudança
                    if (confirm('Deseja cancelar a mudança de email? Seu email permanecerá como ' + profile.email)) {
                      handleCancelEmailChange();
                    }
                  } else {
                    // Se não há email pendente, apenas fecha o dialog
                    setShowVerificationDialog(false);
                    setVerificationCode('');
                  }
                }}
                ...
                className="flex-1 py-3 px-4 bg-neutral-200 hover:bg-neutral-300 text-neutral-700 font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {profile.pendingEmail ? 'Cancelar Mudança' : 'Fechar'}
              </button>
              <button
